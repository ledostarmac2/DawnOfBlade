...
1. Bootstrap the repo-local toolchain:
   ```powershell
   powershell -ExecutionPolicy Bypass -File tools/setup-dev.ps1
   ```
2. Validate the project:
   ```powershell
   powershell -ExecutionPolicy Bypass -File tools/check-dev.ps1
   ```
3. Launch the game:
   ```powershell
...
This installs the .NET 8 SDK, Godot 4.2.2 .NET, and ripgrep, then prepends the vendored SDK to your PATH.
...
`.tools\dotnet\dotnet.exe test DawnOfBlade.sln`.
