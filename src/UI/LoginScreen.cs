...
public partial class LoginScreen : Control
{
    ...
    private Control BuildLoginView()
    {
        ...
        v.AddChild(MakeLabel("SERVER", ColGold, 11, HorizontalAlignment.Left));
        _server = new OptionButton();
        StyleOption(_server);
        _server.AddItem("Northwind Realm");
        _server.AddItem("Emberfall Reach");
        _server.AddItem("Frosthold Keep");
        v.AddChild(_server);
    ...
    private void OnLogin()
    {
        ...
        Session.Server = _server.GetItemText(_server.Selected);
    ...
    private void LoadPrefs()
    {
        ...
        var server = cfg.GetValue("login", "server", string.Empty).AsString();
        for (var i = 0; i < _server.ItemCount; i++)
        {
            if (_server.GetItemText(i) == server)
            {
                _server.Selected = i;
                break;
            }
    ...
    private void SavePrefs()
    {
        ...
        cfg.SetValue("login", "server", _server.GetItemText(_server.Selected));
        cfg.Save(PrefsPath);
    }
    ...
    private void StyleOption(OptionButton option)
    {
        option.CustomMinimumSize = new Vector2(0, 40);
        option.AddThemeStyleboxOverride("normal", FieldBox(ColGoldDim));
        option.AddThemeStyleboxOverride("hover", FieldBox(ColGold));
        option.AddThemeStyleboxOverride("pressed", FieldBox(ColGold));
        option.AddThemeStyleboxOverride("focus", FieldBox(ColGold));
        option.AddThemeColorOverride("font_color", ColText);
        option.AddThemeColorOverride("font_hover_color", ColText);
